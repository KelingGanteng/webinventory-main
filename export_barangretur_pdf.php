...
function generatePDF($htmlContent)
{
    ...
    $dompdf->stream('laporan_barang_retur.pdf', array('Attachment' => 0));
}
...
if (isset($_GET['id'])) {
    ...
    // Ambil data barang retur berdasarkan ID retur
    $sql = $koneksi->query("SELECT barang_retur.*, 
                                   gudang.satuan, 
                            ...
                            WHERE barang_retur.id_retur = '$id_retur'");
    $dataBarangRetur = $sql->fetch_assoc(); // Ambil satu data saja
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="details-list">
                ...
                <li><span class="label">Jumlah Barang:</span> <span class="value">1</span> <!-- Ganti dengan data dinamis --></li>
            </div>
            ...
            <div class="section-content">
                ...
                <div><strong>ID Retur:</strong> {$dataBarangRetur['id_retur']}</div>
                <div><strong>Tanggal Retur:</strong> {$dataBarangRetur['tanggal_retur']}</div>
                <div><strong>Kode Barang:</strong> {$dataBarangRetur['kode_barang']}</div>
                <div><strong>Nama Barang:</strong> {$dataBarangRetur['nama_barang']}</div>
                <div><strong>Kondisi:</strong> {$dataBarangRetur['kondisi']}</div>
                <div><strong>Kerusakan:</strong> {$dataBarangRetur['kerusakan']}</div>
                <div><strong>Jumlah Retur:</strong> {$dataBarangRetur['jumlah']}</div>
                <div><strong>Satuan:</strong> {$dataBarangRetur['satuan']}</div>
                <div><strong>Nama Karyawan:</strong> {$dataBarangRetur['nama_karyawan']}</div>
                <div><strong>Departemen:</strong> {$dataBarangRetur['nama_departemen']}</div>
            </div>
